...
void MonsterScene::update(){
	...
	if(ballCounter >= 20) {
		...
		for(int i=0; i<(int)ofRandom(1, 10); i++) {
			...
			bub.setup(box2d.getWorld(), bx, by, ofRandom(5, 20));
			bub.img = &dotImage;
			balls.push_back(bub);
		}
		...
		if((int)ofRandom(0, 50) == 10 && timeSinceLastBurst > 30) {
			...
			for(int i=0; i<(int)ofRandom(20, 100); i++) {
				...
				MonsterBall bub;
				bub.setPhysics(3.0, 0.3, 0.1); // mass - bounce - friction
				bub.setup(box2d.getWorld(), bx, by, ofRandom(10, 30));
				bub.img = &dotImage;
				balls.push_back(bub);
			}
	...
	for(int i=0; i<tracker->blobs.size(); i++) {
		...
		for(int j=monsters.size()-1; j>=0; j--) {
			...
			if(monsters[j].monsterID == lookID) {
				...
				float scalex = 2.5;//(float)OFFSCREEN_WIDTH / (float)packet.width;
				float scaley = 2.5;//(float)OFFSCREEN_HEIGHT / (float)packet.height;

				sclContour.assign(tracker->blobs[i].pts.size(), ofPoint());
				for(int e=0; e<tracker->blobs[i].pts.size(); e++) {

				//	float ssx = (packet.pts[i][j].x * scalex);
				//	float ssy = (packet.pts[i][j].y * scaley);
