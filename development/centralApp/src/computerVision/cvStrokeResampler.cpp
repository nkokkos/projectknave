...
void cvStrokeResampler::addPoint(float x, float y){
	pts.push_back(ofxVec2f(x,y));
}

void cvStrokeResampler::clear(){
	pts.clear();
}

void cvStrokeResampler::draw(){
	ofSetColor(0,0,0);
	if (pts.size() > 1){
...
float cvStrokeResampler::getLength(){
...
void cvStrokeResampler::resample(int newNumPts){
