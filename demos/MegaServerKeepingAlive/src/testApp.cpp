...
void testApp::setup(){
    ...
    //create the socket and set to send to 127.0.0.1:11998
...
void testApp::update(){
	...
	for(int i = 0; i < TCP.getNumClients(); i++){
		...
		//not dealing with multiple clients yet
		int sent = udpConnection.Send(okMessage.c_str(),okMessage.length());	
		
		if(!sent){
			cout << "UDP couldn't send!" << endl;
		}
	}
}
