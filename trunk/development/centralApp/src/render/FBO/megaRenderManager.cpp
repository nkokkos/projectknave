...
//---------------------------------------------------------------------------
megaRenderManager::megaRenderManager(){
...
//---------------------------------------------------------------------------
void megaRenderManager::allocateForNScreens(int numScreens, int renderWidth, int renderHeight){
    if( numScreens < 1){
        printf("megaRenderManager::allocateForNScreens - you need at lease one screen!\n");
        return;
    }
    width   = renderWidth;
    height  = renderHeight;
	...
	myOffscreenTexture.setClearColor(10,10,10,255);
...
//---------------------------------------------------------------------------
void megaRenderManager::startOffscreenDraw(){
...
//---------------------------------------------------------------------------
void megaRenderManager::endOffscreenDraw(){
    if( nScreens <= 0 ) return;
...
//---------------------------------------------------------------------------
void megaRenderManager::drawInputDiagnostically(float x, float y, float w, float h){
	if( nScreens <= 0 ) return;
...
//---------------------------------------------------------------------------
void megaRenderManager::drawOutputDiagnostically(float x, float y, float w, float h){
    if( nScreens <= 0 ) return;
...
//---------------------------------------------------------------------------
void megaRenderManager::drawScreen(int nScreen, bool debugMode){
	...
	if(debugMode) {
		...
		myOffscreenTexture.draw(0,diff,width, height);
	}
	else {
		myOffscreenTexture.setPoints(inputPositions[nScreen], outputPositions[nScreen]);
		...
		float tx = inputPositions[nScreen][1].x * 600;
		float ty = inputPositions[nScreen][0].y + 600;
		
		//ofCircle(  outputPositions[i][j].x , outputPositions[i][j].y, 5 );

		//ofDrawBitmapString(ofToString(nScreen), tx, ty);
	}
}


//---------------------------------------------------------------------------
void megaRenderManager::resetCoordinates(){
    if( nScreens <= 0)return;
...
//---------------------------------------------------------------------------
void megaRenderManager::loadFromXml(string xmlFileName){
    xmlFile = xmlFileName;
...
//---------------------------------------------------------------------------
void megaRenderManager::reloadFromXml(){
    if(xmlFile == "")return;
...
//---------------------------------------------------------------------------
void megaRenderManager::saveToXml(){
    if(xmlFile != ""){
        saveToXml(xmlFile);
    }
}
...
void megaRenderManager::saveToXml(string xmlFileName){
...
//---------------------------------------------------------------------------
bool megaRenderManager::mouseSelectInputPoint(ofRectangle drawRect, ofPoint mousePoint, float clickRadius){
...
//---------------------------------------------------------------------------
bool megaRenderManager::mouseDragInputPoint(ofRectangle drawRect, ofPoint mousePoint){
    if( nScreens <= 0 ) return false;
...
//---------------------------------------------------------------------------
bool megaRenderManager::mouseSelectOutputPoint(ofRectangle drawRect, ofPoint mousePoint, float clickRadius){
    if( nScreens <= 0 ) return false;
...
//---------------------------------------------------------------------------
bool megaRenderManager::mouseDragOutputPoint(ofRectangle drawRect, ofPoint mousePoint){
    if( nScreens <= 0 ) return false;
