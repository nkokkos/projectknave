...
//--------------------------------------------------------------------------------
void  ofxCvGrayscaleAdvanced::drawBlobIntoMeWithPreallocatedPts( ofPoint * ptsIn, int nPtsIn, int color ){
	
	
	if (nPtsIn > maxPts || nPtsIn <= 0) return;


	for( int i=0; i < nPtsIn ; i++ ) {
	ptsPreallocated[i].x = (int)ptsIn[i].x;
	ptsPreallocated[i].y = (int)ptsIn[i].y;

	}
	int nPts = nPtsIn;
	cvFillPoly( cvImage, &ptsPreallocated, &nPts, 1,
		   CV_RGB(color,color,color) );
