...
void MemoParticle::init(float x, float y) {
	this->x = x;
	this->y = y;
	vx = 0;
	vy = 0;
...
void MemoParticle::updateVertexArrays(int i, float* posBuffer, float* colBuffer) {
	int vi = i * 4;
	posBuffer[vi++] = x - vx;
	posBuffer[vi++] = y - vy;
	posBuffer[vi++] = x;
...
void MemoParticle::drawOldSchool() {
