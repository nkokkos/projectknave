		...
		*/
		...
		for(int i=0; i<children.size(); i++) {
			...
			if(children[i].bDone && !children[i].bMadeLeave) {
				...
				int makeNum = ofRandom(30, 80);
				for(int j=0; j<makeNum; j++) {
					float nj = (float)j / (float)(makeNum-1);
					theLeaves.push_back(TreeLeaf());
					theLeaves.back().pos.x = 10+children[i].p2.x + cos(nj * TWO_PI) * ofRandom(-60, 60);
					theLeaves.back().pos.y = 10+children[i].p2.y + sin(nj * TWO_PI) * ofRandom(-60, 60);
					theLeaves.back().img = img;
