...
void testApp::update(){
	...
	if(weConnected){
		...
		if(message == "OK"){
			//message is ok, so update last recieved
			timeOfLastOKFromServer = ofGetElapsedTimeMillis();			
		}else{
			cout << "Client recieved a non OK message from Server, how is this possible?" << endl;
			cout << "Message recieved is: " << message << endl;
		}
		
		float timeSinceLastOKFromServer =  ofGetElapsedTimeMillis() - timeOfLastOKFromServer;
		
		if(timeSinceLastOKFromServer > 5000){
			//not had anything for five seconds, not good, reset the fucker
			weConnected = false;
			msgRx = "";
		}
	}else{
